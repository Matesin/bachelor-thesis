...
\_def\ctustyleversion{Oct.2025 beta(z)}
...
\_def\fnumbering{\_def\.pgnum##11{}\_def\_advancepageno{\_incr\_pageno}}
\_let\.pgnum=\_empty
...
\.mtdef {errata}       {Errata}            {Errata}         {Errata}
...
\.mtdef {logoctu}      {../images/logo_cvut_en}      {../images/logo_CVUT}      {../images/logo_CVUT}
...
\_let\_fntborder\_tocborder
\_let\_fnfborder\_tocborder
...
\_def\draft{\_showlabels 
     ...
     \_the\_year\_enspace (\_the\.hours:\.Othe\.minutes)\Black}}}}
\_def\.setminutes{%
...
\_def\.pdfinfodata{%
   ...
   \_isinlist\optexversion{Beta}\_iftrue
      \_def\.addinfo##1##2{\_pdfunidef\.tmpb{##2}\_edef\.infolist{\.infolist ##1(\.tmpb)}}%
   \_fi
...
\_def\.specifipage{\_if&\_the\specification&\_else
   ...
   \_the\_ea\specification\_space \vfil\break \_fi}
\_addto\makefront{\.specifipage}

\_def\.declpage{\_if&\_the\declaration&\_else
    \_insertoutline{\_mtext{declaration}}
    \_the\_ea\declaration\_space \_vfil\_break \_fi}
\_addto\makefront{\.declpage}
...
\_def\.erratapage{
	...
	\_insertoutline{\_mtext{errata}}
	\_hrule height0pt \_vskip0pt plus 1filll
	\_line{
		\.bf\_thefontsize[19]
		\_hss\Blue{\_mtext{errata}}\_hss
	}
	\_nobreak\_medskip
	\_line{
		\.sethsizethanks
		\_vtop to\.frontht{
			\_hrule width\_hsize height0pt
			\_firstnoindent
			Version of this text: {\sbf \_the\_year-0\_the\_month-\_the\_day\_space\_the\.hours:\_the\.minutes\_space CEST}
			\_nobreak\_medskip
			\_firstnoindent
			\_the\errata
			\_vfil
...
\_addto\makefront{%
	\_footline={\_hss\.ssr\_thefontsize[11]%
	\_ifx\.pgnum\_empty \_ea\_romannumeral\_ea-\_else \_ea\_the \_fi \_pageno\_hss\.drafttext}
	\.erratapage \_vfil\_break
}
...
%%% ???
\_def\.sethsizethanks{\_advance\_hsize by-30mm
\_emergencystretch=2em \_righthyphenmin=2 \_hbadness=5000 \_penalty0 }
...
\_def\.thankspage{
	\_insertoutline{\_mtext{thanks}}
	\_hrule height0pt \_vskip0pt plus 1filll
	\_line{
		\.bf\_thefontsize[19]
		\_hss\Blue{\_mtext{thanks}}\_hss
	}
	\_nobreak\_medskip
	\_line{
		\.sethsizethanks
		\_vtop to\.frontht{\_hrule width\_hsize height0pt \_firstnoindent\_the\thanks \_vfil}
	}
}
\_addto\makefront{%
   \_footline={\_hss\.ssr\_thefontsize[11]%
      \_ifx\.pgnum\_empty \_ea\_romannumeral\_ea-\_else \_ea\_the \_fi \_pageno\_hss\.drafttext}
   \.thankspage \_vfil\_break
}
...
\_def\.totfpages{
   \_setbox\.totbox=\_vbox{\.sethsizefront \_parskip=0pt \_iindent=9mm
                           \.totlist
                           \_bigskip {\Blue \_hrule height 3pt}\_bigskip
                           \.toflist \_vfill}
   \.prepbox\.totbox
   \_let\.tmp=\_do
   \_ifx\.toflist\_empty \_ifx\.totlist\_empty \_let\.tmp=\_relax \_fi\_fi
   \_ifx\.tmp\_do
      \_insertoutline{\_ifx\.totlist\_empty \_else \_mtext{tables}\_fi/%
                      \_ifx\.toflist\_empty \_else \_mtext{figures}\_fi}
      \_setbox\.leftbox=\_vsplit\.totbox to\.frontht
      \_setbox\.rightbox=\_vsplit\.totbox to\.frontht
      \.frontpage[\_ifx\.totlist\_empty \_else\_mtext{tables}\_fi/%
                  \_ifx\.toflist\_empty \_else\_mtext{figures}\_fi]
      \_loop \_let\.tmp=\_relax
            \_ifvoid\.totbox \_else \_setbox\.leftbox=\_vsplit\.totbox to\.frontht
                                 \_let\.tmp=\_do \_fi
            \_ifvoid\.totbox \_else \_setbox\.rightbox=\_vsplit\.totbox to\.frontht \_fi
            \_ifx\.tmp\_do \_vfil\_break \.frontpage[/] \_repeat
   \_fi
}
...
\_addto\makefront{%
   ...
   \_ifx\.pgnum\_empty \_pdfcatalog{/PageLabels<</Nums[0<</S/r>>\_the\_pageno<</S/D>>]>>}\_fi
   \_egroup
   \.setlinespacing
   \.trysavetoner
   \.pgnum\_pageno=1
...
\_def\_printchap#1{\_vfill\_supereject \.myvakat \_vglue1cm
...
\_def\.printapp#1{\_vfill\_supereject \.myvakat \_vglue1cm
  \_headline={\_hfil\.nextheadline}
...
\_def\slides{\_par
  ...
  \_hoffset=12mm
  \_def\_wideformat{\_margins/1 (263,148) (12,15,10,3)mm }\_public \wideformat ;
  \_everypar={}
...
\_def\.counterslide{%
  ...
  \_vtop{\_baselineskip=18pt
     \_null\.slideslist}%
}
\_def\.prepslidepages{\_openref\_def\.slideslist{}%
  \_if?\_lastpage \_else
     \_ifnum\_lastpage>22
         \_edef\.coef{\_expr[3]{22/\_lastpage}}
         \_edef\.slideslist{\_baselineskip=\.coef\_baselineskip}\_fi
     \_tmpnum=0
...
\_def\.printpagenum{%
  \_raise4pt\_vbox to0pt{\_vss
      \_llap{\_smallbf\_the\_pageno \_ifnum\_slidelayer=0\.maxlayer\_else+\_fi}\_vss}%
}

\_def\.pagerectangle#1{\_kern-2pt
   \_ifx\.coef\_undefined \_else \_afterfi{\_transformbox{\_pdfscale{\.coef}{\.coef}}}\_fi{%
   \_let\Blue=\_relax \_let\_ilinkcolor=\_relax
...
\_def\totf{\_let\.tofpages=\.totfpages}
...
beta(w) Jun.2024  \draft: hours, minutes added
beta(x) Jul.2024  \totf for comact list of tables+figures added
beta(y) May.2025  test of \hexstring removed: bug in \pdfinfo fixed
beta(z) Oct.2025  \wideformat corrected, when more pages: pageslider is resized
