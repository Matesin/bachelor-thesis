...
To further extend Endler's overview, this section briefly examines the post-quantum (PQ) security considerations of the CTAP protocol. As quantum computing technology advances, it poses a significant long-term threat to currently used public-key cryptographic schemes. Although no quantum computer is currently capable of breaking them, the issue is important to acknowledge~\cite[enwiki:1349019275].

\nl The version of CTAP implemented in this project (CTAP 2.2) does not provide native PQ security, as it relies on conventional public-key cryptographic primitives that could be compromised by sufficiently capable quantum adversaries. Nevertheless, the current protocol architecture is considered to be PQ ready. In particular, its PIN/UV authentication model requires active user verification and physical presence, while the protocol itself can be adapted to stronger cryptographic primitives without a fundamental redesign. Therefore, a full quantum-resistant operation could be achieved by replacing the current asymmetric mechanisms with post-quantum key encapsulation and signature mechanisms, provided that such changes are introduced consistently across both CTAP and WebAuthn within the FIDO2 framework~\cite[cryptoeprint:2022/1029].
