...
\nl In response, organizations have increasingly {\sbf adopted multi-factor authentication (MFA}), which supplements passwords with additional verification factors such as one-time codes, authentication applications, or trusted devices. The adoption of MFA has grown significantly; for example, 83\% of organizations required MFA for all access to IT in 2024~\cite[jumpcloud-sme-2024]. Despite variations across surveys, it is clearly observable that MFA has become a popular safety measure.

\sec[passkeys] Passkeys

\nl Despite the growing usage of MFA, Ing. Martin Endler argues in his diploma thesis that many existing authentication methods
``do not provide sufficient security and hinder the user experience''
...
\nl A distinguishing {\sbf characteristic} of most commercially available hardware security keys is their {\sbf closed-source firmware}. Open-source alternatives do exist but remain rare: the first publicly known example was SoloKeys (developed 2018–2024, now discontinued), followed by OpenSK by Google and Nitrokey. The present work contributes to this space by providing the first open-source implementation of FIDO2 over NFC in a C-based security key.

\sec[fido2-framework] The FIDO2 Framework

\nl The FIDO2 framework, developed by the FIDO Alliance in collaboration with the W3C, is the current {\sbf industry standard} for {\sbf strong, phishing-resistant and passwordless authentication}. It is an open framework that uses public-key cryptography.
...
* {\sbf CTAP2:} The current standard for FIDO2, which improves user verification methods.

\enditems

\nl A {\sbf detailed breakdown} of this framework, including a detailed description of the CTAP protocol, can be found in the {work of Endler~\cite[endler-thesis]}, along with a more comprehensive insight into the problematic of hardware security keys, including the cryptographic mechanisms that are commonly used.
